...
def dataprofile(vector):
    datas=map(float,vector)
    datas.sort()
    mx=datas[-1]
    mn=datas[0]
    md=datas[len(datas)/2]
    return mn,md,mx
...
def rpgMatrix(inputM):
    ...
    for dataline in inputM[1:]: #dataline is geneline
        ...
        for i in range(1,len(dataline)):
            rpmed_data=float(dataline[i])/medList[i-1]
            newline.append(rpmed_data)
