...
#       1. in information gain calculation, the bin number were fixed as 4!!!1
#       2. If average acc achieve 100, stop grid search#
#
#############################
#   Module: model_cv_rbf_ig.py
#	Author: Jie Shen  @ NCTR/FDA
#	Date: Aug. 24, 2012.
#
#   Description:
...
-i labelfile -j datafile -t testdatafile -d outputDIR"""
    ...
    if is_win32:
        Path=r"C:\JShen\Research\EDKB\ER\DCAEA\121105\\"
        outpath=r"C:\JShen\Research\EDKB\ER\DCAEA\121105\\"
    else:
...
def getSVMinput(labelmatrix):              #for training set
    ...
    for line in labelmatrix:
        ...
        Y.append(line[1])
        newtitleindex.append(sampletitles.index(int(line[0])))
...
# Give the default parameters
                ...
                if 100-maxacc>0.0000001:
                    ...
                    for innerrun in range(0,maxinnerrun):
                        for i in range(0,fold):
                            ...
                            p_label, p_acc, p_val=svm_predict(innertestingY,selectedtestingX,m,'-b 1 -q')
                            acc_list.append(p_acc[0])

                    acc_ave=average(acc_list)
                    outacc.write("grid %d\t%f\n" % (grid,acc_ave))
                    if acc_ave>maxacc:
                        param_n=n
    ...
    p_label, p_acc, p_val=svm_predict(testingY,selectedtestingX,m,'-b 1 -q')
